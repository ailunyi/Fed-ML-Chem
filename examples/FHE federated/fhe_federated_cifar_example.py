"""
FHE Federated Learning Example for CIFAR Dataset
Uses Fed-ML-Lib's built-in FHE implementation
"""
import torch
import flwr as fl
...
def client_fn(cid: str):
    """Create an FHE Flower client using the library implementation"""
    batch_size = 32
    ...
    # Create CIFAR model with FHE support
    model = create_model(
    ...
    # Use the library's FHEFlowerClient
    return FHEFlowerClient(
        ...
        device=device,
        batch_size=batch_size,
        save_results=f"results/fhe_federated_cifar/client_{cid}/",
        ...
        fhe_scheme="CKKS",
        fhe_layers=["classifier"],
        context_path=f"fhe_context_client_{cid}.pkl"
    )
...
def evaluate_fn(server_round, parameters, config):
    ...
    # Save results
    os.makedirs("results/fhe_federated_cifar/server/", exist_ok=True)
    classes = ['plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']
    save_all_results(
        train_true=y_true,
        train_pred=y_pred,
        ...
        training_history={},
        classes=classes,
        results_path="results/fhe_federated_cifar/server",
        config={},
        file_suffix=""
    )
...
def main():
    ...
    # Create initial model with FHE support
    ...
    print("=" * 60)
    print("FHE FEDERATED LEARNING - CIFAR DATASET")
    print("Using Fed-ML-Lib's Built-in FHE Implementation")
    print("=" * 60)
    print(f"Device: {device}")
    print(f"Clients: 4")
    print(f"Rounds: 10")
    print(f"Dataset: CIFAR-10")
    print(f"Architecture: CNN with FHE")
    ...
    os.makedirs("results/fhe_federated_cifar", exist_ok=True)
    ...
    print(f"FHE federated training completed in {time.time() - start_time:.2f} seconds")
